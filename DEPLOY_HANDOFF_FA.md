...
> «ارزش روز کار انجام‌شده» صفر می‌ماند. «هزینه بروز باقیمانده» و «پیش‌بینی هزینه نهایی» از
> فاکتورها محاسبه می‌شوند و به پیشرفت نیازی ندارند (بخش ۶، قاعدهٔ «تسویه از دفتر»). مقدارِ
> اجراشده در سطح تخصیص باید از ماژول پیشرفت سایت منتشر شود؛ تا آن روز، صفرِ ارزش اجراشده
> یعنی «اندازه‌گیری نشده»، نه «تمام شده».
...
- **سه نوع منبع (از مهاجرت ۰۰۳۸، 2026-09-26):** `material`، `work`، `general_cost`. همان سه
  نوعی که MS Project دارد (MATERIAL / WORK / COST). `work` یعنی «نیرو و تجهیزات»: اکیپ و
  دستگاه یک نوع‌اند و به ساعت خریده می‌شوند. مقادیر قدیمی `labor` و `equipment` در دیتابیس
  بازنویسی نمی‌شوند و سرویس آن‌ها را `work` می‌خواند (`domain/resource_types.py`). منابع WORK
  فایل زمان‌بندی خودکار `work` با واحد `hour` می‌شوند؛ مرحلهٔ طبقه‌بندی دستی حذف شد.
  **اثر روی API:** `resourceType` در breakdown و کلیدهای ماهانه سه مقدار دارند، نه چهار.
- **قاعدهٔ «تسویه از دفتر» (از 2026-09-26):** «هزینه بروز باقیمانده» و «بودجه موردنیاز تا
  ...
  **نیرو و تجهیزات**: مبلغ هر فاکتور بر قیمتِ معتبر در **تاریخ همان فاکتور** (`price_versions`
  با `effective_from ≤ invoice_date`، وگرنه قیمت برآورد) تقسیم می‌شود تا «ساعت تسویه‌شده» به
  دست آید، و `(بازنگری‌شده − تسویه‌شده) × قیمت روز` باقیمانده است. برای **هزینهٔ عمومی**:
  `برآورد − پرداختی`. پیشرفت فقط «ارزش روز کار انجام‌شده» را می‌سازد و در هیچ باقیمانده‌ای
  نقش ندارد. پرداخت نیرو یا تجهیزاتی که در تاریخش هیچ قیمتی نبوده و برآورد هم قیمت ندارد
  هشدار `INVOICE_PRICE_MISSING` می‌گیرد؛ راه‌حل ثبت قیمت با تاریخ اثر است، نه ثبت پیشرفت.
...
| ۶ | جداول `msp_*` سایت فقط درصد دارند، نه مقدار | با provider سایت به‌تنهایی، فقط «ارزش روز کار انجام‌شده» صفر می‌ماند؛ «هزینه بروز باقیمانده» و پیش‌بینی نهایی از فاکتورها محاسبه می‌شوند (بخش ۳-۲ و ۶) |
