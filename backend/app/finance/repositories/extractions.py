...
class PsycopgExtractionRepository:
    ...
    async def confirm_with_invoice(self, scope, draft, confirmed_fields, invoice, extraction_audit_id, invoice_audit_id, at):
        fields = [{"key": field.key, "extractedValue": field.extracted_value,
            "confirmedValue": field.confirmed_value, "confidence": field.confidence,
        ...
        try:
            async with self.db.transaction():
             async with self.db.cursor(row_factory=dict_row) as cursor:
                ...
                await cursor.execute("INSERT INTO finance_audit_events(id,organization_id,project_id,actor_user_id,action,entity_type,entity_id,after_values,occurred_at) VALUES(%s,%s,%s,%s,'invoice.confirmed','invoices',%s,%s,%s)", (invoice_audit_id,scope.organization_id,scope.project_id,scope.actor_user_id,invoice.id,Jsonb({"source":invoice.source,"status":"confirmed","finalAmountIrr":str(invoice.final_amount_irr),"extractionDraftId":str(draft.draft_id)}),at))
