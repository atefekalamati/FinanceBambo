        ...
        async with self.db.cursor(row_factory=dict_row) as cursor:
            ...
            await cursor.execute("""SELECT EXISTS (
                ...
                WHERE l.organization_id=%s AND l.project_id=%s AND l.deleted_at IS NULL
                  AND (l.created_at AT TIME ZONE 'Asia/Tehran')::date>%s
                UNION ALL
                SELECT 1 FROM estimate_line_source_completions c
                JOIN estimate_lines l ON l.id=c.estimate_line_id
                  AND l.organization_id=c.organization_id AND l.project_id=c.project_id
                WHERE c.organization_id=%s AND c.project_id=%s AND l.deleted_at IS NULL
                  AND (c.completed_at AT TIME ZONE 'Asia/Tehran')::date>%s
                  AND (l.original_quantity IS NULL OR l.original_unit_price_irr IS NULL)
            ) AS uncertain""",(scope.organization_id,scope.project_id,as_of,
                              scope.organization_id,scope.project_id,as_of))
