...
class PsycopgFinanceResourcesRepository:
    ...
    def _line(row, revisions=()):
        return EstimateLine(row["id"], row["organization_id"], row["project_id"], row["resource_id"],
            row["activity_external_id"], row["assignment_external_id"],
            # The effective original: the line's own column, or the documented completion
            # of the version it was mapped from. `.get` because one caller reads a row that
            # selected neither -- there the column IS the answer.
            row.get("effective_original_quantity", row["original_quantity"]),
            row["revised_quantity"],
            row.get("effective_original_unit_price_irr", row["original_unit_price_irr"]),
            row["source"], row["created_by"], row["created_at"],
            ...
            row.get("mpp_quantity"), row.get("mpp_unit"),
            row.get("mpp_unit_confidence"), row.get("mpp_cost_irr"),
            row.get("original_value_source"))
    ...
    async def list_estimate_lines(self, scope):
        async with self._connection.cursor(row_factory=dict_row) as c:
            await c.execute("""SELECT l.*, """ + effective_original_quantity("l") + """ effective_original_quantity, """ + effective_original_price("l") + """ effective_original_unit_price_irr, """ + original_value_source("l") + """ original_value_source, COALESCE((SELECT r.new_quantity FROM estimate_revisions r WHERE r.organization_id=l.organization_id AND r.project_id=l.project_id AND r.estimate_line_id=l.id ORDER BY r.revision DESC LIMIT 1),CASE WHEN fr.resource_type='general_cost' THEN """ + effective_original_price("l") + """ ELSE """ + effective_original_quantity("l") + """ END) revised_quantity,COALESCE((SELECT max(r.revision) FROM estimate_revisions r WHERE r.organization_id=l.organization_id AND r.project_id=l.project_id AND r.estimate_line_id=l.id),0) current_revision,(SELECT sum(il.final_line_amount_irr*i.financial_effect_sign) FROM invoice_lines il JOIN invoices i ON i.organization_id=il.organization_id AND i.project_id=il.project_id AND i.id=il.invoice_id WHERE il.organization_id=l.organization_id AND il.project_id=l.project_id AND il.estimate_line_id=l.id AND i.status IN ('confirmed','voided','corrected')) actual_cost_irr,(SELECT m.source_assignment_units FROM finance_mpp_rows m WHERE m.organization_id=l.organization_id AND m.project_id=l.project_id AND m.source_assignment_uid=l.source_assignment_uid AND m.source_version_id=""" + active_source_version_for("l") + """) mpp_quantity,(SELECT m.normalized_unit FROM finance_mpp_rows m WHERE m.organization_id=l.organization_id AND m.project_id=l.project_id AND m.source_assignment_uid=l.source_assignment_uid AND m.source_version_id=""" + active_source_version_for("l") + """) mpp_unit,(SELECT m.unit_confidence FROM finance_mpp_rows m WHERE m.organization_id=l.organization_id AND m.project_id=l.project_id AND m.source_assignment_uid=l.source_assignment_uid AND m.source_version_id=""" + active_source_version_for("l") + """) mpp_unit_confidence,(SELECT m.source_assignment_cost_irr FROM finance_mpp_rows m WHERE m.organization_id=l.organization_id AND m.project_id=l.project_id AND m.source_assignment_uid=l.source_assignment_uid AND m.source_version_id=""" + active_source_version_for("l") + """) mpp_cost_irr FROM estimate_lines l JOIN finance_resources fr ON fr.organization_id=l.organization_id AND fr.project_id=l.project_id AND fr.id=l.resource_id WHERE l.organization_id=%s AND l.project_id=%s AND l.deleted_at IS NULL ORDER BY l.created_at,l.id""", (scope.organization_id,scope.project_id));lines=await c.fetchall()
