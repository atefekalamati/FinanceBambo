...
async def preview_prices(projectId:str,request:Request,file:UploadFile=File(...),currencyUnit:str|None=Form(None,pattern="^(IRR|TOMAN)$")):
    """currencyUnit is recorded on the import batch for audit only; each row carries its own mandatory currency column, which is what normalization uses."""
    return await _preview_import(projectId,request,"prices",file,currencyUnit)
