...
35. `0035_the_sheet_value_survives_a_failed_normalisation` (head)
...
**۰۰۳۰ سومین مهاجرتِ داده‌جابه‌جاکنِ این زنجیره است** (در کنار ۰۰۲۰ و ۰۰۲۵) و به همین
دلیل `scripts/reconcile_ledger.py` هر بازه‌ای شامل آن را `refused-data-migration` اعلام
می‌کند. یک ستون `legacy_status` به `estimate_lines` اضافه می‌کند و سه CHECK:

```
legacy_status IS NULL          زمان‌بندی این ردیف را توضیح می‌دهد
'legacy_unlinked'              نمی‌دهد، پس از جمع‌های MPP-derived کنار گذاشته می‌شود
'approved_exception'           نمی‌دهد، ولی کسی تصمیم گرفته به حساب بیاید
```

**Backfill صادق است و هویت نمی‌سازد**: دقیقاً روی ردیف‌هایی نوشته می‌شود که هر دو ستون
uid خودشان NULL است — بازگویی چیزی که ردیف از قبل می‌گوید، نه ادعایی دربارهٔ منشأ آن.
هیچ Resource UID، Assignment UID یا Task UID ساخته نمی‌شود و هیچ ردیفی با نام تطبیق داده
نمی‌شود.

**هیچ ردیفی حذف یا غیرفعال نمی‌شود.** هر ۱۲۰ ردیف زنده و خواندنی می‌مانند. دوتای‌شان
(«برآورد مصالح مورد نياز» و «بتن ريزي سقف») توسط سطرهای فاکتور ارجاع داده می‌شوند و
فاکتور شاهدِ کاری است که کسی انجام داده — این از هر نظری دربارهٔ منشأ ردیف بادوام‌تر است.
علامت فقط تعیین می‌کند ردیف وارد کدام **جمع** شود، نه اینکه وجود داشته باشد یا نه.

سومین CHECK (`estimate_lines_state_their_provenance`) همان چیزی است که ردیف بی‌منشأ بعدی
را متوقف می‌کند — در خود پایگاه داده، از هر مسیری که برسد. با یک INSERT آزمایشی ثابت شد.

**۰۰۳۱ فقط شکل می‌سازد و هیچ داده‌ای نمی‌نویسد.** سه ستون nullable/پیش‌فرض‌دار روی
`finance_unit_conversion_rules` و یک CHECK که می‌گوید «اقرارشده ولی بی‌نام» حالت مجازی
نیست.

**چرا لازم شد:** `validate_scope` هر قانون بین‌بُعدی را در دامنه‌ای وسیع‌تر از یک listing
رد می‌کرد — و استدلالش درست است: «۱ شاخه = ۲۲ کیلوگرم» وزن‌کشیِ یک محصول است. ولی دامنهٔ
باریکی که به آن ارجاع می‌داد (`provider_item`) تا وقتی listingی به سطر وصل نشده قابل
نوشتن نیست، و روی پروژهٔ ممیزی‌شده **۸۳۲ از ۸۳۵ سطر هیچ component ندارند**. یعنی کاربر به
دری فرستاده می‌شد که قفل بود.

حالا ادعا ممکن است، ولی **نام روی آن می‌نشیند**: چه کسی و چه زمانی. پیش‌فرض `false` است،
پس هر فراخوانی‌ای که به‌روز نشود دقیقاً مثل امروز رد می‌شود.

**Backfill لازم نیست و ممکن هم نیست**: هیچ قانون بین‌بُعدیِ وسیعی در جدول وجود ندارد چون
نوشتنش تا امروز مجاز نبوده.
...
(دامنه‌دار، تأییدشونده، نسخه‌دار و آمادهٔ فرمول)، و جدول مسائل تبدیل واحد. هیچ ستونی
پیش‌فرض ندارد — پیش‌فرضِ صفر برای «مقدار روز» یعنی برآورد روزِ هر ردیف پروژه در روز
اجرای مهاجرت صفر شود. Upgrade → Downgrade → Upgrade روی یک پایگاه دادهٔ یک‌بارمصرف
آزموده شد و تنها یک head باقی ماند.
...
### پیامدی که renumbering دارد و باید بدانید

هر دیتابیسی که **زیر زنجیرهٔ پیشین** با `0019` مهر خورده باشد (وقتی والدش ۰۰۱۷ بود)، حالا
می‌گوید head است و DDL ۰۰۱۸ را **ندارد** — و `alembic upgrade head` روی آن هیچ کاری
نمی‌کند، چون از نظر alembic همان‌جاست که باید باشد.

اندازه‌گیری‌شده روی `bambo_fixedcost_e2e`: ledger `0019`، و قید
`progress_snapshot_refs_status_check` همان `(ready, failed)` مانده — یعنی ۰۰۱۸ اعمال نشده
در حالی که ledger ادعای head دارد.

**هیچ دیتابیس مشترکی درگیر نیست.** سرشماری هر ۳۷ دیتابیس محلی: دیتابیس ارائه روی `0016`
است و `0019` فقط در کپی‌های دورریختنیِ همین کار. برای هر کپیِ این‌طور: **دوباره بسازید، یا
۰۰۱۸ را روی آن اعمال کنید.** این همان دلیلی است که برای مقصد اصلاً پیش نمی‌آید — مقصد روی
۰۰۱۶ است و ۰۰۱۷ و ۰۰۱۸ و ۰۰۱۹ را به ترتیب اجرا می‌کند.
