...
18. `0019_finance_mpp_fixed_cost` (head)

شمارهٔ ۰۰۱۸ عمداً خالی است. شاخهٔ `origin/backend-finance` یک 0018 دارد که واژگان CHECK
همین 0017 را اصلاح می‌کند و هنوز اینجا merge نشده؛ برداشتن آن شماره یعنی دو revision با
یک شناسه، که Alembic اصلاً نمی‌پذیرد. وقتی آن revision وارد شد، `down_revision` این یکی
به `"0018"` تغییر می‌کند و زنجیره دوباره خطی می‌شود.
