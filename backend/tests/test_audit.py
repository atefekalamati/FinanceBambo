...
class Repository:
    def __init__(self,row):self.row=row;self.scope=None
    async def list(self,scope):self.scope=scope;return [self.row]
...
class AuditTests(unittest.IsolatedAsyncioTestCase):
    async def test_lists_only_repository_scoped_immutable_events(self):
        ...
        result=await FinanceAuditService(repository).list(scope)
