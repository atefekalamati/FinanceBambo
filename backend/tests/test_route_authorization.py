...
class ScopedQueryTests(unittest.TestCase):
    ...
    def statements(self, tree):
        ...
        # `"..." + helper() + "..."` is the same failure in different syntax, and it
        # arrived when the price ladder moved into a shared module: the chunk carrying
        # `FROM finance_mpp_rows` and the chunk carrying `WHERE m.organization_id=%s`
        # became separate Constant nodes, and the first was reported as an unscoped query
        # over a finance table. The statement was scoped the whole time.
        def concatenated(node):
            """The string constants of a `+` chain, left to right."""
        ...
        for node in ast.walk(tree):
            if not (isinstance(node, ast.BinOp) and isinstance(node.op, ast.Add)):
                continue
            parts = concatenated(node)
            if len(parts) < 2 or any(id(p) in folded for p in parts):
                continue
            text = "".join(p.value for p in parts)
            # Fold ONLY when folding repairs something: a chain whose literal halves
            # together name the tenant. Everything else is left exactly as it was, so the
            # runtime-composed statements in `invoices.py` and `unit_conversion_rules.py`
            # keep reaching the `WHERE`-suffix rule that has always covered them. A repair
            # that quietly changed the verdict on unrelated queries would not be a repair.
            if "organization_id" not in text:
                continue
            folded.update(id(p) for p in parts)
            chains.append(text)
        for text in chains:
            # A chain whose literal halves already name the tenant is SCOPED, whatever the
            # helpers add -- that is `items_and_estimates`, whose FROM and WHERE ended up
            # in different chunks when the price ladder moved to a shared module.
            #
            # A chain that does NOT name it is composed in the older sense: `invoices.py`
            # builds `... WHERE " + filters + " ORDER BY ...` and its scope arrives at
            # runtime. Folding those two cases together would have let the second through
            # unexamined, so they are still told apart -- by what the literals say, which
            # is the only evidence this check has.
            yield text, False
        for node in ast.walk(tree):
            if isinstance(node, ast.JoinedStr):
                ...
                yield text, True
                  ...
                  and id(node) not in inside_fstring and id(node) not in folded):
