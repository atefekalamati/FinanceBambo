...
| شاخص | وضعیت فعلی |
|---|---|
| پیشرفت وزنی Frontend تا نسخه Mock-complete | حدود ۷۱٪ |
| فاز فعال | فاز ۶ — فایل، OCR/Voice و AI Review |
| کار بعدی قطعی | شروع پردازش OCR/Voice و صفحه بازبینی استخراج |
| تست‌های فعلی | ۸۰ تست موفق |
| صفحات فعال | امور مالی، تنظیمات مالی، اقلام و متره، قیمت‌ها، خوراک پیشرفت، فاکتورها و ورود تصویر/صدا |
| وابستگی مسدودکننده فعلی | Endpoint فهرست فایل‌های پروژه در Backend فعلی وجود ندارد؛ توسعه Mock مسدود نیست |
...
| ۶. فایل، OCR/Voice و AI Review | ۱۰٪ | در حال انجام | ۳۰٪ | ۳٪ |
...
یادداشت Integration: Backend نسخه فعلی `POST /files` و `GET /files/{fileId}` دارد، اما Endpoint فهرست فایل‌های پروژه تعریف نشده است. فهرست فعلی فقط در Adapter آزمایشی و در طول نشست نگهداری می‌شود؛ تیم Backend باید قرارداد Pagination/Filter فهرست را تعیین و پیاده‌سازی کند. Frontend هیچ تغییری در Backend نداده است.
