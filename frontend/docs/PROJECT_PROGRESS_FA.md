...
## قیمت‌گذاری قلم از چند مصالح — ۲۰۲۶-۰۹-۱۵

- یک فعالیت، مصالح نیست. «کانال کنی» ۷٬۵۱۶٫۴ مترمکعب ترانشه است و میلگرد و لوله و آجر مصرف می‌کند، در حالی که فایل MPP هیچ‌کدام را نام نمی‌برد — چون برنامهٔ زمان‌بندی «کار» را توصیف می‌کند نه فهرست مصالح. بنابراین هر قلم با یک **فهرست** مصالح قیمت‌گذاری می‌شود و هزینهٔ روز قلم، جمع آن‌هاست.
- هر جزء، یک محصول واقعی از قیمت روز، یک واحد رسمی و «چقدر» را نگه می‌دارد. «چقدر» دو خوانش دارد و خوانش درست حدس‌زدنی نیست: `به ازای هر واحد فعالیت` (۸۰ کیلوگرم میلگرد برای هر مترمکعب) یا `مقدار کل برای این قلم` (۱٬۲۰۰ متر لوله در کل). روی یک ترانشهٔ ۷٬۵۱۶ مترمکعبی این دو به اندازهٔ ضریب ۷٬۵۱۶ فرق می‌کنند، پس هیچ پیش‌فرضی انتخاب نمی‌شود.
- **ضریب مصرف، تبدیل واحد نیست.** «۸۰ کیلوگرم بر مترمکعب» واقعیتی دربارهٔ طراحی همین فعالیت است، نه دربارهٔ واحدها؛ `unit_conversions` برای آن پرسیده نمی‌شود و نمی‌تواند پاسخ دهد.
- جدول ریز برآورد **هیچ ستونی نگرفت**: «قیمت روز» جمع اجزا را نشان می‌دهد، «منبع» برای یک مصالح نام محصول و برای چند مصالح «چند مصالح (n قلم)» را، و «واحد» واحد خودِ فعالیت می‌ماند.
- جزئی که قیمت‌گذاری نشده، **صفر شمرده نمی‌شود**: دلیلش را به فارسی می‌گوید و ردیف در کنار جمعِ اجزای حل‌شده می‌نویسد «۱ از ۲ قلم مصالح قیمت‌گذاری شده است». جمعی که بخشی از آن ناقص است، واقعی و ناتمام است و ردیف هر دو را می‌گوید.
- همه‌چیز Append-only است: ویرایش یک جزء، نسخهٔ تازه می‌سازد و حذف آن، نسخهٔ غیرفعال؛ هیچ ردیفی پاک یا بازنویسی نمی‌شود، چون گزارشی که هفتهٔ پیش صادر شده با همان نسخه محاسبه شده بود. Migration `0025` هر «اتصال تک‌محصولی» موجود را به‌عنوان نخستین جزء همان ردیف منتقل می‌کند (`per_msp_unit` با مصرف ۱)، که دقیقاً معنای مدل قبلی است، پس هیچ ردیف موجودی عددش تغییر نکرد.
- **یک نقص واقعی پیدا و رفع شد**: ذخیرهٔ هر جزء، وضعیت قیمت جدول را تازه می‌کند و آن تازه‌سازی صفحه را دوباره می‌کشید و پنل باز را از سند بیرون می‌برد. پنل بعد از نخستین مصالح ناپدید می‌شد و کلیک بعدی روی دکمهٔ یک ردیف دیگر می‌نشست. این را مرورگر واقعی نشان داد، نه تست.
- آزمون‌ها: ۲۸ تست دامنهٔ محاسبه، ۴۶ تست سرویس، ۲۷ تست Frontend؛ کل ۱۶۷۸ تست Backend و ۶۴۱ تست Frontend موفق. راستی‌آزمایی مرورگر روی `bambo_canonical_test` انجام شد: دو مصالح روی ردیف «کانال کنی» ثبت شد، پس از Reload باقی ماند، جمع برابر مجموع اجزای حل‌شده بود، جزء بدون واحد مبدأ «—» ماند و نه صفر، هزینهٔ MSP ردیف دست‌نخورده ماند و Console هیچ خطایی نداد.

## خلاصه اقلام و برآورد در نمای کلی — ۱۴۰۵/۰۶/۱۸

- کارت فقط‌خواندنی «اقلام و برآورد» با سه ردیف از همان workspace صفحه گزارش اقلام به نمای کلی اضافه شد و مسیر «مشاهده همه» به `/report-items` متصل است.
- فضای قبلی کارت قیمت‌های روز بدون تغییر ابعاد ردیف، میان دو کارت خلاصه قیمت و خلاصه اقلام تقسیم شد؛ وضعیت خطا و خالی هر منبع مستقل نمایش داده می‌شود.
- در عرض `48rem` و پایین‌تر، ارتفاع هر دو کارت از محتوای داخلی تعیین می‌شود و اسکرول عمودی داخلی حذف شده است.
- صحت نحو و قراردادهای UI با مجموعه تست Node بررسی شد؛ اتصال HTTP واقعی همچنان وابسته به Runtime سایت میزبان است.

## حذف مسیرهای تکراری از بخش‌های گزارش مالی — ۱۴۰۵/۰۶/۱۸

- کارت‌های «گزارش مالی سطح ۱»، «فاکتورها»، «جدول قیمت‌ها» و «تنظیمات نمایش» از `/work-areas` حذف شدند، چون هر چهار مقصد از بخش متناظر خود در نمای کلی گزارش مالی لینک مستقیم دارند.
- کارت‌های «گزارش وضعیت مالی»، «گزارش دوره‌ای» و «جدول اقلام و برآورد» باقی ماندند تا هیچ مقصدی تنها مسیر دسترسی خود را از دست ندهد.
- تست ناوبری اکنون هم نبود کارت تکراری و هم وجود لینک جایگزین هر مقصد در نمای کلی را کنترل می‌کند؛ ۲۱ تست مرتبط موفق‌اند.

## ساده‌سازی نوار بالای نمای کلی — ۱۴۰۵/۰۶/۱۸

- اطلاعات مبنای محاسبه، نسخه و تاریخ گزارش، وضعیت، فایل مبدأ و زمان ورود Snapshot دیگر در نمای مشتری رندر نمی‌شوند و فقط به‌صورت یک لاگ ساختاریافته در Console در دسترس توسعه‌دهنده‌اند.
- اختلاف احتمالی بین شناسه Snapshot انتخاب‌شده و Snapshot استفاده‌شده در پاسخ گزارش نیز در Console هشدار داده می‌شود.
- سه مسیر ناوبری موجود در `finance-basis__meta` بدون تغییر در همان نوار باقی ماندند.
- تست‌های متمرکز نمای کلی و ماژول‌های مشترک: ۲۹ تست موفق.
