...
export function validateInvoiceLine(values, target) {
  ...
  if (target?.targetType === "general_cost") {
    ...
    if (!/^\d+$/.test(amountIRR) || !/[1-9]/.test(amountIRR)) errors.amountIRR = "مبلغ خط باید عدد صحیح و مثبت ریال باشد.";
  ...
  if (!/^\d+$/.test(unitPriceIRR) || !/[1-9]/.test(unitPriceIRR)) errors.unitPriceIRR = "قیمت واحد باید عدد صحیح و مثبت ریال باشد.";
...
export function validateInvoiceAdjustments(values) {
  ...
  for (const key of ["discountIRR", "taxIRR", "shippingIRR", "otherCostsIRR"]) {
    ...
    if (!/^\d+$/.test(value)) errors[key] = "مبلغ تعدیل باید عدد صحیح و نامنفی ریال باشد.";
